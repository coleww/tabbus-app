...
export function Tuning({data, setData}: TuningProps) {
  ...
  const handleKeyDown = useCallback(e => {
    if (e.key === 'Enter') {
      ...
      const newTuning = editedString.match(/(\w#|\w)/g)?.filter(note => ALL_NOTES.includes(note)).reverse() ?? [];
      setData(newTuning);
      setIsEditing(false);
    }
  }, [setData]);
  ...
  return (
    <div data-id='tuning'>
      {isEditing ? <input className='' type='text' value={editedTuning} onChange={handleChange}
        onKeyDown={handleKeyDown}/>
        : <button className='' onClick={() => {
          setIsEditing(true);
          setEditedTuning(displayTuning);
        }}>{displayTuning}</button>
      }
    </div>
  );
}
